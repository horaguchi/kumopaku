...
func _ready():
	...
	var end_idx = start_idx + Global.unlocked_skills_count
	var unlocked_skills = ItemData.SKILLS.slice(start_idx, end_idx)

	if not unlocked_skills.is_empty():
		var translated_unlocked_skills = unlocked_skills.map(func(s): return tr(s))
		all_unlocked_str += "  ".join(translated_unlocked_skills) + "  "
	...
	_tw.tween_callback($AudioStreamPlayer.play)
